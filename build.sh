...
cd ${ROOT_DIR}/app/lal && \
  go build -ldflags " \
    -X 'github.com/q191201771/nezha/pkg/bininfo.GitCommitID=`git log --pretty=format:'%h' -n 1`' \
    -X 'github.com/q191201771/nezha/pkg/bininfo.BuildTime=`date +'%Y.%m.%d.%H%M%S'`' \
    -X 'github.com/q191201771/nezha/pkg/bininfo.BuildGoVersion=`go version`' \
  " -o ${ROOT_DIR}/bin/lal

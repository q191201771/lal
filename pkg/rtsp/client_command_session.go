...
func (session *ClientCommandSession) writeOptions() error {
	...
	method := ctx.Headers.Value(HeaderPublic)
...
func (session *ClientCommandSession) writeOneSetup(setupUri string) error {
	...
	session.sessionId = strings.Split(ctx.Headers.Value(HeaderSession), ";")[0]

	rRtpPort, rRtcpPort, err := parseServerPort(ctx.Headers.Value(HeaderTransport))
	if err != nil {
		return err
	}
...
func (session *ClientCommandSession) writeCmdReadResp(method, uri string, headers map[string]string, body string) (ctx nazahttp.HttpRespMsgCtx, err error) {
	for i := 0; i < 2; i++ {
        ...
        //目前只处理第一个
		session.auth.FeedWwwAuthenticate(ctx.Headers.Value(HeaderWwwAuthenticate), session.urlCtx.Username, session.urlCtx.Password)
	}
